...
const RATE_OPTIONS = [0, 3, 5, 12, 18, 28];
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 sm:p-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
        <div>
          <label htmlFor="pseo-amount" className="block text-sm font-medium text-gray-700 mb-2">
        ...
        <div>
          <label htmlFor="pseo-rate" className="block text-sm font-medium text-gray-700 mb-2">
            GST Rate (%)
          </label>
          {lockRate ? (
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 sm:p-6">
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-4">
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wide">Base</p>
          ...
          {supplyType === "intra" ? (
            <>
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wide">CGST</p>
                <p className="text-lg font-bold text-gray-900">{formatCurrency(result.cgst)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wide">SGST</p>
                <p className="text-lg font-bold text-gray-900">{formatCurrency(result.sgst)}</p>
              </div>
            </>
          ) : (
            <div className="col-span-2">
              <p className="text-xs text-gray-500 uppercase tracking-wide">IGST</p>
              <p className="text-lg font-bold text-gray-900">{formatCurrency(result.igst)}</p>
            </div>
          )}
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wide">Total</p>
            <p className="text-lg font-bold text-primary">{formatCurrency(result.total)}</p>
          </div>
        </div>
      </div>
      ...
      <button
        type="button"
        onClick={handleCopy}
        className="mt-4 inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors min-h-[44px]"
      >
